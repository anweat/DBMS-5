...
public:
    ...
    std::string parseIdent() {
        ...
        for (auto t : allowed)
            if (cur().type == t) return advance().value;
        throw DBException(ErrorCode::SQL_SYNTAX_ERROR,
    ...
    std::shared_ptr<WhereExpr> parseComparison() {
        ...
        ExprOp op;
        switch (cur().type) {
            case TokenType::EQ:    advance(); op = ExprOp::EQ;  break;
            case TokenType::NEQ:   advance(); op = ExprOp::NEQ; break;
            case TokenType::LT:    advance(); op = ExprOp::LT;  break;
            case TokenType::LE:    advance(); op = ExprOp::LE;  break;
            case TokenType::GT:    advance(); op = ExprOp::GT;  break;
            case TokenType::GE:    advance(); op = ExprOp::GE;  break;
            default:
                throw DBException(ErrorCode::SQL_SYNTAX_ERROR,
    ...
    std::vector<SelectColumn> parseSelectColumns() {
        ...
        do {
            ...
            if (tryAgg(AggFunc::COUNT, TokenType::COUNT)) continue;
            if (tryAgg(AggFunc::SUM,   TokenType::SUM))   continue;
            if (tryAgg(AggFunc::MAX,   TokenType::MAX))   continue;
            if (tryAgg(AggFunc::MIN,   TokenType::MIN))   continue;
            if (tryAgg(AggFunc::AVG,   TokenType::AVG))   continue;
    ...
    ASTNodePtr parseStatement() {
        switch (cur().type) {
            ...
            case TokenType::BEGIN:  return parseBegin();
    ...
    ASTNodePtr parseCreate() {
        ...
        switch (cur().type) {
            case TokenType::DATABASE: return parseCreateDatabase();
            case TokenType::TABLE:    return parseCreateTable();
            case TokenType::UNIQUE:
            case TokenType::INDEX:    return parseCreateIndex();
            default:
                throw DBException(ErrorCode::SQL_SYNTAX_ERROR,
                                  "Expected DATABASE/TABLE/INDEX after CREATE");
        }
    }
            ...
            if (check(TokenType::PRIMARY)) {
                ...
                for (auto& col : n->def.columns)
                    for (const auto& pk : pkCols)
                        if (col.name == pk) { col.primaryKey = true; col.nullable = false; }
            }
    ...
    ASTNodePtr parseDrop() {
        ...
        switch (cur().type) {
            case TokenType::DATABASE: return parseDropDatabase();
            case TokenType::TABLE:    return parseDropTable();
            case TokenType::INDEX:    return parseDropIndex();
            default:
                throw DBException(ErrorCode::SQL_SYNTAX_ERROR,
                                  "Expected DATABASE/TABLE/INDEX after DROP");
        }
    }
    ...
    ASTNodePtr parseSelect() {
        ...
        if (check(TokenType::ORDER)) {
            ...
            do {
                ...
                if (match(TokenType::DESC))       ob.ascending = false;
                else { match(TokenType::ASC); ob.ascending = true; }

...
static const std::unordered_map<std::string, TokenType> KEYWORDS = {
    ...
    {"DATETIME", TokenType::DATETIME_KW},
...
std::vector<Token> Lexer::tokenize(const std::string &sql)
{
    ...
    while (true)
    {
        ...
        case '<':
            ...
            {
                sc.advance();
                tokens.push_back({TokenType::LE, "<=", ln, cl});
            }
            else if (n == '>')
            {
                sc.advance();
                tokens.push_back({TokenType::NEQ, "<>", ln, cl});
            }
            else
                tokens.push_back({TokenType::LT, "<", ln, cl});
            break;
        case '>':
            if (n == '=')
            {
                sc.advance();
                tokens.push_back({TokenType::GE, ">=", ln, cl});
            }
            else
                tokens.push_back({TokenType::GT, ">", ln, cl});
            break;
        case '!':
            if (n == '=')
            {
                ...
                tokens.push_back({TokenType::UNKNOWN, "!", ln, cl});
            break;
        case '=':
            tokens.push_back({TokenType::EQ, "=", ln, cl});
            break;
        case '+':
            tokens.push_back({TokenType::PLUS, "+", ln, cl});
            break;
        case '-':
            tokens.push_back({TokenType::MINUS, "-", ln, cl});
            break;
        case '*':
            tokens.push_back({TokenType::STAR, "*", ln, cl});
            break;
        case '/':
            tokens.push_back({TokenType::SLASH, "/", ln, cl});
            break;
        case '%':
            tokens.push_back({TokenType::PERCENT, "%", ln, cl});
            break;
        case '(':
            tokens.push_back({TokenType::LPAREN, "(", ln, cl});
            break;
        case ')':
            tokens.push_back({TokenType::RPAREN, ")", ln, cl});
            break;
        case ',':
            tokens.push_back({TokenType::COMMA, ",", ln, cl});
            break;
        case ';':
            tokens.push_back({TokenType::SEMICOLON, ";", ln, cl});
            break;
        case '.':
            tokens.push_back({TokenType::DOT, ".", ln, cl});
            break;
        default:
            tokens.push_back({TokenType::UNKNOWN, std::string(1, c), ln, cl});
            break;
        }
    }
